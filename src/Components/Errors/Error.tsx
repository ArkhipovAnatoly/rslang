import './Error.css';

const Error = () => (
    <div className="wrapper">
    <div className="composition">
      <div className="layer-0 shelf">
        <div className="shelf__side_left" />
        <div className="shelf__side_bottom" />
        
        <div className="case case_1">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_2">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_3">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_4">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_5">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_6">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_7">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_8">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_9">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_10">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_11">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_12">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_13">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_14">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_15">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_16">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_17">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_18">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_19">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_20">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_21">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_22">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_23">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_24">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_25">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_26">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_27">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_28">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_29">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_30">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_31">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_32">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_33">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_34">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_35">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_36">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_37">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_38">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_39">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_40">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_41">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_42">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_43">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_44">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_45">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_46">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_right" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        <div className="case case_47">
          <div className="case__front" />
          <div className="case__top" />
          <div className="case__label case__label_left" />
          <div className="case__right" />
          <div className="case__number">
            №2428506
          </div>
        </div>
        
        <div className="glow">
          <div className="glow__bottom" />
          <div className="glow__top" />
          <div className="glow__ball glow__ball_1" />
          <div className="glow__ball glow__ball_2" />
          <div className="glow__ball glow__ball_3" />
          <div className="glow__ball glow__ball_4" />
          <div className="glow__ball glow__ball_5" />
          <div className="glow__ball glow__ball_6" />
          <div className="glow__ball glow__ball_7" />
          <div className="glow__ball glow__ball_8" />
          <div className="glow__ball glow__ball_9" />
          <div className="glow__ball glow__ball_10" />
        </div>
        
        <div className="shelf__side_front" />
        <div className="shelf__side_right" />
        
        <div className="shelf__handle_top" />
        <div className="shelf__handle_front" />
        <div className="shelf__handle_right" />
      </div>
      <div className="layer-1 shadow" />
      <div className="layer-2 numbers">
        <div className="numbers__item numbers__item_1">4</div>
        <div className="numbers__item numbers__item_2">0</div>
        <div className="numbers__item numbers__item_3">4</div>
      </div>
    </div>
  </div>
  )

export default Error;